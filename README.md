...
В одной группе рядом с обязательными утверждениями `assert` можно заранее описывать будущие изменения как `proposal`:
...
Каждый элемент должен содержать ровно одно поле: `assert` или `proposal`. Названия элементов уникальны в пределах группы независимо от типа. Предложение отображается в локальном редакторе, может иметь `description`, не сопоставляется с автотестами и не влияет на показатели покрытия. Чтобы сделать будущее поведение обязательным, замените у элемента `proposal` на `assert`.
...
Для формата JUnit можно опционально указать свойство из которого будет извлекаться название тест кейса.
