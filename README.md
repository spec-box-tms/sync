...
#### Ссылки, изображения и HTML в Markdown

В локальном интерфейсе `serve` можно ссылаться на файлы проекта, показывать изображения и встраивать HTML-документы. Это работает при просмотре фичи, её требований и сравнении ревизий.

Пример описания в YAML:

```yaml
description: |-
  [Открыть документ](docs/readme.txt)

  ![Схема](images/schema.svg)

  [Открыть HTML-отчёт](reports/report.html)

  ![Встроенный HTML-отчёт](reports/report.html)

  ![Схема с пробелами в пути](<images/схема процесса.svg>)
```

Локальные пути отсчитываются от каталога, из которого запущена команда `serve`, а не от расположения YAML-файла или текущей страницы интерфейса. Например, при запуске из корня репозитория `images/schema.svg` указывает на файл в его каталоге `images`. Файлы должны находиться внутри каталога запуска; абсолютные пути и переходы через `..` не поддерживаются.

Локальный адрес автоматически преобразуется в `/api/files?path=…`. Пробелы, кириллица и символы `#`, `?`, `&`, `%` кодируются как часть пути: вручную кодировать их не нужно. Для пути с пробелами используйте угловые скобки, как в примере выше. В локальном адресе `#` и `?` обозначают символы имени файла, а не фрагмент или query-параметры.

Адреса, начинающиеся с `http://` или `https://` без учёта регистра, сохраняются без изменений. Все остальные адреса обрабатываются как локальные пути.

Markdown-изображение с расширением `.html` или `.HTML` отображается как `iframe` на всю доступную ширину с пропорцией **16:9**. Для внешнего HTTP(S)-адреса расширение определяется по пути до query и fragment. Обычная ссылка на HTML остаётся ссылкой. Встроенный документ работает в `sandbox` без разрешения на выполнение скриптов.

Чтобы показать сам синтаксис вставки в тексте требования, оберните его в обратные кавычки:

```yaml
specs-unit:
  Документация:
    - assert: При отображении `![Отчёт](reports/report.html)` создаётся iframe
```
...
В одной группе рядом с обязательными утверждениями `assert` можно заранее описывать будущие изменения как `propose`:
...
Каждый элемент должен содержать ровно одно поле: `assert` или `propose`. Названия элементов уникальны в пределах группы независимо от типа. Предложение отображается в локальном редакторе, может иметь `description`, не сопоставляется с автотестами и не влияет на показатели покрытия. Чтобы сделать будущее поведение обязательным, замените у элемента `propose` на `assert`.
...
Чтобы добавить в выгрузку ФТ информацию из отчётов Jest или JUnit, добавьте в корень [конфигурационного файла](#формат-конфига) секцию `"jest"` или `"JUnit"`. В каждой секции можно указать один объект настройки либо массив объектов. Одиночная нотация остаётся поддерживаемой и не требует миграции в массив.

Каждая настройка содержит путь `reportPath` и массив `keys`. Для формата JUnit можно дополнительно указать `property` — свойство, из которого извлекается название тест-кейса.

В одной конфигурации можно одновременно использовать Jest и JUnit, в том числе несколько отчётов каждого формата. CLI применяет все заданные отчёты.
...
Для нескольких наборов тестов укажите массив настроек. В этом примере используются два отчёта Jest и два отчёта JUnit:

```js
{
    "jest": [
        {
            "reportPath": "test-results/unit-jest.json",
            "keys": ["featureTitle", "groupTitle", "assertionTitle"]
        },
        {
            "reportPath": "test-results/e2e-jest.json",
            "keys": ["featureCode", "assertionTitle"]
        }
    ],
    "JUnit": [
        {
            "reportPath": "test-results/unit-junit.xml",
            "keys": ["featureTitle", "groupTitle", "assertionTitle"]
        },
        {
            "reportPath": "test-results/e2e-junit.xml",
            "property": "description",
            "keys": ["featureCode", "assertionTitle"]
        }
    ]
...
Если файл из `reportPath` отсутствует, синхронизация проекта и команда `serve` продолжают работу. В `serve` отсутствие отчёта отображается в диагностике с уровнем `warning`.

Для каждого обязательного ФТ (`assert`), описанного в yml файлах проекта, будет сформирован уникальный идентификатор на основе поля `"keys"`. Если в отчете jest есть тест, полное имя которого совпадает с идентификатором ФТ, то считается, что проверка этого ФТ — автоматизирована. Элементы `propose` при сопоставлении с Jest и JUnit игнорируются.
...
- `"Любой текст"` - значение может быть использовано как разделитель
...
```js
{
    ...
    // настройки для сопоставления ФТ с отчётами Jest:
    // один объект или массив таких объектов
    "jest": {
