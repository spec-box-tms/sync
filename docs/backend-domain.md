...
Все пути файлов в ответах и запросах относительны к каталогу проекта и
используют `/` как разделитель. Поля, не описанные в модели запроса, делают
запрос некорректным.
...
YAML-файла; он обязательно присутствует только при чтении текущей фичи и
передаётся в `PUT`. У снимка из Git-ревизии его нет.
...
При сохранении клиент передаёт полную редактируемую структуру:
...
interface UpdateFeatureRequest {
  ...
  groups: Array<{
    ...
    assertions: Array<{
      title: string;
      description?: string;
      isAutomated?: boolean; // игнорируется
    }>;
  }>;
  optimisticLock: string;

  // Допустимо для повторной отправки модели формы, но backend игнорирует.
  filePath?: string;
}
```

`isAutomated` внутри утверждения и `filePath` игнорируются. Путь существующей
фичи изменить нельзя.

Backend переводит модель в YAML без потерь по следующему правилу:
...
| `groups[].title` и `assertions` | ключ и список `specs-unit` |
| `assertions[].title` | `assert` |

Комментарии и неизвестные YAML-поля в MVP не сохраняются.
...
Возвращает текущую `Feature` с обязательным `optimisticLock`.
...
Принимает `UpdateFeatureRequest`. `:code` выбирает существующий файл; поле
`code` в теле может задать фиче новый код. До записи сервер сверяет
`optimisticLock` с текущим содержимым файла. При успехе он полностью
пересчитывает проект и возвращает `ProjectSnapshot`.
...
`409 Conflict` не имеет тела. Сервер не меняет файл при `400`, `404` или
`409`. Неразрешённые ссылки и повторные `assert` не являются ошибками записи:
они сохраняются и попадают в `diagnostics` нового снимка.
