...
const withProposalServer = async <T>(
  run: (context: {
    url: string;
  ...
  try {
    ...
    await exec('git', ['commit', '-m', 'Proposal feature'], { cwd: project.root });
    const revision = (
...
specTest(
  ...
  'Локальный API',
  'ProjectSnapshot и ответы текущей и исторической фичи сохраняют type каждого assert и proposal',
  () =>
    withProposalServer(async ({ url, snapshot, revision }) => {
...
specTest(
  ...
  'Предложения в локальном редакторе',
  'Локальный API',
  'Proposal в ответе локального API имеет type равный proposal и isAutomated равный false',
  () =>
    withProposalServer(async ({ url }) => {
...
specTest(
  ...
  () =>
    withProposalServer(async ({ snapshot }) => {
...
specTest(
  ...
  'Предложения в локальном редакторе',
  'Локальный API',
  'Граф зависимостей ProjectSnapshot содержит связи из названия и description proposal',
  () =>
    withProposalServer(async ({ snapshot }) => {
