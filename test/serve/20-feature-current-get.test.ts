...
specTest('serve-feature-get', 'GET /api/features/:code', 'Успешный ответ', 'Каждый propose ответа содержит type равный propose и isAutomated равный false', () => withServer(async (url) => {
  const current = await feature(url);
